...
def send_to_leaderboard(data, leaderboardUrl):
    print(data,leaderboardUrl)
    try:
        r = httpx.post(leaderboardUrl, json = data)
        print(r.status_code)
    except httpx.RequestError:
        return "Error", r.status_code

def worker_queue(username, result, guesses, leaderboardUrl):
    redis = Redis()
    queue = Queue(connection=Redis())
    registry = FailedJobRegistry(queue=queue)
    data = {"guesses": guesses,'result': result,'username':username}
    result = queue.enqueue(send_to_leaderboard, data, leaderboardUrl)
    for failed in registry.get_job_ids():
        job = Job.fetch(failed, connection=redis)
        print(failed)                    
...
async def guess(gameId):
    ...
    if game[4] == 1:
        #message queue
        worker_queue(auth.username, "You_Won", 6-game[3],"http://127.0.0.1:5400/reportgame")
    ...
    #message queue
    worker_queue(auth.username, "You_Lost", 6-game[3],"http://127.0.0.1:5400/reportgame")
...
@app.route("/webhookCreation", methods=["POST"])
async def register_client():

    res =  await request.get_json()
    leaderboardUrl = res.get("leaderboardUrl")
    clientName = res.get("clientName")
    db_select = await _get_db()
    db_insert = await _get_primary_db()

    
    is_available = await db_select.fetch_all(
    "SELECT * FROM webhook WHERE clientName=:clientName",
    values={"clientName": clientName},
    )

    if len(is_available) == 0:
        await db_insert.execute(
            "INSERT INTO webhook(leaderboardUrl, clientName) VALUES(:leaderboardUrl, :clientName)",
            values={"leaderboardUrl": leaderboardUrl, "clientName":clientName}
            )
        return {
            "Success": "Registration of client is successfull",
        }, 200  
    else:
        return {
